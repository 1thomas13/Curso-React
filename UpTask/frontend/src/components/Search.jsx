...
export const Search = () => {
    ...
    const filter = busqueda == '' ? [] : projects.filter(project => {
        return project.name.toLowerCase().includes(busqueda.toLocaleLowerCase())
    })
        ...
        <Transition.Root show={search} as={Fragment} afterLeave={ ()=>setBusqueda('') }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSearch}>
                ...
                <Combobox
                    ...
                    onChange={ (project) =>{ 
                        handleSearch()
                        return (navigate(`/proyectos/${project._id}`)) 
                    }}
                >
                    ...
                    {filter.length > 0 && (
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {filter.map(project=>{
                                return <Combobox.Option className={({active}) => classNames('cursor-pointer select-none px-4 py-2',active && 'bg-sky-600 text-white')} value={project} key={project._id}>{project.name}</Combobox.Option>
                            })}
                        </Combobox.Options>
                    )}
                    </Combobox>
